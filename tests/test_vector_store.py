...
from app.services.vector_store import query_similar_chunks, store_embedded_chunks


def test_store_and_query_embedded_chunks(tmp_path) -> None:
    chroma_path = str(tmp_path / "chroma_test_db")
    collection_name = "test_log_chunks"
    ...
    assert results["ids"][0][0] == "chunk_1"

    assert len(results["documents"][0]) == 1
    assert "PaymentService timeout" in results["documents"][0][0]

    assert len(results["metadatas"][0]) == 1
    assert results["metadatas"][0][0]["services"] == "PaymentService"